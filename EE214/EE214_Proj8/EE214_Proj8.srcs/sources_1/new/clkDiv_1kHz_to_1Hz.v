...
assign terminalCount = (counter == terminatingCount);

always @ (posedge(i_clk), posedge(i_rst)) // incrementing this clock on the posedge of the 1kHz couner
begin
    if (i_rst) begin counter <= 0; end // if we enable reset count gets set to 0
    else if (terminalCount) begin counter <= 0; end // if we hit the terminal count reset counter to divide it down to 1kHz
...
begin
    ...
    else if (terminalCount) begin o_divClk = !o_divClk; end	// TFF with the second counters terminal count as the trigger
end
endmodule
