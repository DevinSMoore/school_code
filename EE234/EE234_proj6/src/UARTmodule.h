...
Description: This will contain all of the functions necessary for UART communications
*******************************************************************************************/
#include <stdint.h>
...
#define UART1_CONTROL		*((volatile uint32_t *)0xE0001000) 		// bit[5] = Tx Disable; bit[4] = Tx EN; bit[3] = Rx Dis; bit[2] = Rx En; bit[1]=Tx reset (1=RS); [0]:Rx reset (1=reset)
#define UART1_MODE	 		*((volatile uint32_t *)0xE0001004) 		// bit[9:8]: ChannMode(00 for normal); [7:6] #StopBits (00 for 1); [5:3] parity (1XX for no parity); [2:1] dataLen(0X:8bits); [0] MasterClk (0 for 100MHz);

// Interrupt Registers
#define UART1_EN_INTR 		*((volatile uint32_t *)0xE0001008)		// 1=En; [10]:Tx Near Full; [4]:TxFIFOfull; [3]:TxFIFOempty; [2]:RxFIFOfull; [1]:RxFIFOempty; [0]:Rx Trigg Int
#define UART1_DIS_INTR 		*((volatile uint32_t *)0xE000100C)		// Same mapping as above; write 1 to disable
#define UART1_INTR_MASK		*((volatile uint32_t *)0xE0001010)		// Same mapping but read to check if INT is EN; 1=EN,0=NOT EN
#define UART1_INTR_STATUS	*((volatile uint32_t *)0xE0001014)		// Same mapping, any enable interrupts will set the values in this reg when their conditions are met; Write a 1 to a field to clear
...
#define UART1_TX_TRIG_VAL	*((volatile uint32_t *)0xE0001044)		// Same as above but for the transmit FIFO
...
#define UART1_STATUS		*((volatile uint32_t *)0xE000102C)		// [11]: Tx State Mch. Active (1=Actv); [10]:RX State Mch; [4]:TxFull; [3]:TxEmpty; [2]:RxFull; [1]:RxEmpty; [0]:Rx level trigger bit; if FIFO is above level set this will be set
#define UART1_FIFO			*((volatile uint32_t *)0xE0001030)		// This holds the FIFO data in the least significant byte; read the recieve FIFO, and write to the transmit FIFO using this same offset; 1 byte only please and thank you; also use LDRB to make sure we nab the correct info
